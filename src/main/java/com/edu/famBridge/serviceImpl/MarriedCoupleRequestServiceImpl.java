...
public class MarriedCoupleRequestServiceImpl implements MarriedCoupleRequestService {
        ...
        @Autowired
        private final MarriedCoupleRequestRepository marriedCoupleRequestRepository;
        @Autowired
    ...
    @Override
    public MarriedCoupleRequestDTO getMarriedCoupleRequestByUniqueId(long requestId){
        MarriedCoupleRequestDTO marriedCoupleRequestDTO = new MarriedCoupleRequestDTO();

        try{
            MarriedCoupleRequest marriedCoupleRequestById = (MarriedCoupleRequest) marriedCoupleRequestRepository.findByRequestId(requestId)
                    .orElseThrow(() -> new RuntimeException("Request not found"));

            marriedCoupleRequestDTO.setEmail(marriedCoupleRequestById.getEmail());
            marriedCoupleRequestDTO.setFirstName(marriedCoupleRequestById.getFirstName());
            marriedCoupleRequestDTO.setRequestDate(marriedCoupleRequestById.getRequestDate());
            marriedCoupleRequestDTO.setRequestId(marriedCoupleRequestById.getRequestId());
            marriedCoupleRequestDTO.setGramaNiladhariDivision((marriedCoupleRequestById.getGramaNiladhariDivision()));
            marriedCoupleRequestDTO.setStatusCode(200);
            marriedCoupleRequestDTO.setMessage("Married couple request for Id: " + requestId + " found successfully.");


        }

        catch (Exception e){
            marriedCoupleRequestDTO.setStatusCode(500);
            marriedCoupleRequestDTO.setMessage("Error occurred: " + e.getMessage());
        }

        return marriedCoupleRequestDTO;
    }
